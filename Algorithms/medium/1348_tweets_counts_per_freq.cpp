...
public:
    ...
    void recordTweet(string tweetName, int time) {
        tweets[move(tweetName)].push_back(time);
    }
    
    vector<int> getTweetCountsPerFrequency(const string& freq, const string& tweetName, int startTime, int endTime) {
        int period = time_dict[freq];
        int bins_num = (endTime - startTime) / period + 1;
        vector<int> bins(bins_num, 0);
        
        for (int time : tweets[tweetName]) {
            if (time < startTime || time > endTime) continue;
            int bin_num = (time - startTime) / period;
            ++bins[bin_num];
        }
        return bins;
    }
...
private:
    ...
    unordered_map<string, int> time_dict{ {"minute", 60}, {"hour", 3600}, {"day", 86400} };
};

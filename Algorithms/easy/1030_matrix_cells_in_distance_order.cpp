...
Algo1: make bfs for graph from matrix 

Algo2: naive: put all values to res vector and sort it
...
public:
    vector<vector<int>> allCellsDistOrder(int R, int C, int r0, int c0) {
        ...
        while (!q.empty()) {
            ...
            // up
            if (r != 0 && !visited.count({r-1,c})) {
                q.emplace(r-1, c);
                visited.insert({r-1,c});
            }
            // down
            if (r+1 != R && !visited.count({r+1, c})) {
                q.emplace(r+1, c);
                visited.insert({r+1,c});
            }
            // left
            if (c != 0 && !visited.count({r, c-1})) {
                q.emplace(r, c-1);
                visited.insert({r,c-1});
            }
            // right
            if (c+1 != C && !visited.count({r, c+1})) {
                q.emplace(r, c+1);
                visited.insert({r,c+1});
            }
        }
